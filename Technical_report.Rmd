...
The Whole dataset has a 13,647,309 obsarvation and 48 observation. In order to keep the recommender simple, in this part I used only customer codes and products information from the dataset which is only 25 attributes.
...
Our data consist of Customer code (Cust_code) and 24 products/Accounts which are binary value. According to the documentation, a Account value equal to 0 represents the customer doesn't use this Account and equal 1 use this Account.

## Exploring and Cleaning Data


```{r na, warning=FALSE, error=FALSE, echo=FALSE }
...
We can see that some Accounts have NA values 

```{r rm_NA&duplicates, warning=FALSE, error=FALSE, echo=FALSE}
...
# Let's see how many total products customers have 

df.unique$totalproducts<-rowSums(df.unique[,2:25], na.rm = T)
table(df.unique$totalproducts)

barplot(table(df.unique$totalproducts), xlab="Number of Products", las=1.5)
...
#We see that dataset is imbalanced around 91 : 9 ratio, where 91% of dataset has a 0 value and 9% - 1 value
...
ratingmat <- as.matrix(df.clean[,-1]) #remove Cust_code 

rownames(ratingmat)<-df.clean[,1]

ratingmat<-ratingmat[sample(nrow(ratingmat),10000),] #randomly select 5000 users
...
image(as.matrix(similarity_items), main = "Products similarity")
```
...
image(ratingmat[1:20, 1:24], main = "Heatmap of the first 20 rows and 24 columns", useraster=T)
```
...
*dgCMatrix* is a similarity matrix created by the model. Its dimensions are 24 x 24, which is equal to the number of items. The heatmap of items show that many values are equal to 0. The reason is that each row contains mostly k (6) elements that are greater than 0. The number of non-null elements for each column depends on how many times the corresponding product was included in the top k of another product. Thus, the matrix is not neccessarily simmetric, which is also the case in our model. 

The chart of the distribution of the number of elements by column shows there are a few products that are similar to many others. 
...
For each user, the algorithm extracts its used products. For each product, it identifies all its similar items, starting from the similarity matrix. Then, the algorithm ranks each similar item and identifies the top 3 recommendation:
...
dim(as.matrix(recc_matrix))
as.matrix(recc_matrix)[1:10,]
...
Let's take a look at the first ten users:
...
```{r times_per_products, warning=FALSE, message=FALSE, echo=FALSE}
...
```
We see that "Acc_Particular", "Acc_Payroll" and "Acc_Direct_Dedit" are most often reccomended items.
...
First, I re-define the evaluation sets, build models, IBCF and UBCF, and create matrices with predicted ratings.
...
Now, I compute the accuracy measures for each user and vizualise F-measure for each model IBCF and UBCF.
...
According to the Precision/Recall chart we can see that User-Based Collaborative Filtering model gives higher results amonge the others. Moreover, it gives quit similar results with *Jaccard*, *Cosine* and *Pearson* distances.
...
Based on the ROC curve's and the precision/recall plots, with different values of *nn* n_nearest_neighbors we are getting quite similar results. So, for my model I will choose default value which is equal 30. 
