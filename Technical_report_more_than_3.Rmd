...
*Scenario 1(Remoing customers who use less than 4 prduct)*
...
We can see actual correlation values. Acc_nom_Pensions and Acc_nom_Payroll and Acc_Payroll products are the ones are highly correlated. As I don't have detailed description of each product, I cannot assume the reasons why they're highly correlated.
...
As there are too many users, the first chart is hard to read. The second chart is built zooming in on the first rows and columns.
...
I compare the models by building a chart displaying their ROC curves and prevision/recall curves.
...
Based on the ROC curve's and the precision/recall plots, with different values of *nn* n_nearest_neighbors we are getting quite similar results. But with *nn* = 60 we get a little bit better results. So, for my model I will choose *nn* equal 60. 
